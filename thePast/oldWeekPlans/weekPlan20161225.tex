...
\item \tiny anger about taxi --> say you are anxious 
          \item \tiny  Confused about talking too much --> laugh it off.
        \end{itemize}
      \end{block}
      %-- Block 1-5
...
\url{https://www.bnl.gov/newsroom/news.php?a=11892}
    ...
    \item \tiny tm tips.
    \end{enumerate}
        {{\tiny \bf News:} M: | S: trump+obamacare not replaced,
          aleppo | E: Fallon:  w: {m: {60s r a}}  t:{60s s a} {w: {70s
              s a}} {r: {70s s a}} {f: {70s s a   } } {a :{70s s a} } 
          {u: {  }  } }
  ...
  \begin{block}{Running ToDo} % find a better name 
    ...
    \item \tiny Work: talk to Todd \te{1 hour}.

    \item \tiny Work(docs): read on raven. 
     
    \item \tiny Work(docs): 
    \item \tiny Work(setup): get git repo up + Work(setup): get machine access +
      Work(setup): get project access + Work(setup): set up pictures
      for git repo + Work(setup): put up picture for slack.
    ...
    \item \tiny Mgmt:Spaces: flight booking for home = Mgmt:Spaces:
      choose between Amtrak + Mgmt:Spaces: choose flight for 182 on
      24th + Mgmt:Spaces: consider driving back + Mgmt:Spaces: 
    ...
    \item \tiny Mgmt:Spaces:\decision/ travel for this week = 

go home - 
Pro: Mom and Dad sees you, better attitude, see counselor.
Con: nothing to do. - lose thirty days - big deal. 

Pro: Dad can stress you out less.
Con: Dad comes in the way.
 
    \item \tiny \doneTaskNoItem{Mgmt:webpresence: fb like to sadhna} +
      Mgmt:webpresence: fb like to sumi +
      Mgmt:webpresence: send message to Shalaka. \doneTask{EI: call Kavita.} 
