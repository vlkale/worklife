  ...
  \begin{column}{0.6\linewidth}
    ...
    \end{block}
      \begin{block}{Running ToDo} % find a better name 
        \begin{enumerate}
        \doneTask{ Comm: message to Nitin Mama about meeting.} \te{10
          minutes}.  
      \doneTask{Comm: text to Renuka}
      
      \doneTask{Comm: reply to Shalaka}. 
      \doneTask{Comm: follow-up to Namrata}. 
      \item \tiny Mgmt:Spaces: get flight for labor day
        weekend. \te{30 minutes} \pr{IU} \dl{Thursday} = 
       \doneTask{ Work:rel: register for OpenMPCon} 
       \doneTask{Mgmt:Spaces: get eye exam. } 
      \doneTask{ Work:admin: give information for OpenMPCon to
        Claire.} \te{1 hour} 
      \doneTask{Work: rel: e-mail to Bronis.} \te{1 hour}.  
     \item \tiny \textbf{Curr} $\rightarrow$ 
     \item \tiny Work: make slides for meeting next week with
       Harshitha. \te{1 hour}. 
     \item \tiny Work:rel: reply to Amanda. \te{10 minutes}.   
     \item \tiny Mgmt:Spaces: get medicines from CVS.
     \item \tiny \doneTaskNoItem{Work:admin: get airline ticket for
       OpenMPCon for arrival} + Work:admin: get airline ticket for
       OpenMPCon for departure. 
      \item \tiny Work:admin: get hotel for OpenMPCon. \te{30 minutes} 

      \item \tiny Work:rel: email to Abid \te{1 hour}. \pr{High}           
        Hi Abid, I hope that you are doing well. 
        
      \item \tiny Work: rel: email to Bronis. \te{1 hour}. 
Hi Bronis,
...
The presentation will propose a way to specify allowing loop scheduling techniques to be added more easily while being closely tied with loop scheduling libraries of well-established runtimes for OpenMP. 

Adding schedule kind named ‘user’ for the schedule clause of the OpenMP standard and an standard methodology to define a user-defined scheduling strategy. The proposed method of defining the OpenMP implementation of loop scheduling strategies is based on an understanding of open-source compilers such as LLVM and GCC but is intended to highlight the algorithm strategy rather than implementation details.

The schedule for the technique in OpenMP's standard that is based on the user specification rather predefined schedules ‘static’, ‘dynamic’, ‘guided’ . 

An expected outcome from doing so is more rapid development of loop scheduling strategies for OpenMP in the context of scientific and engineering application codes.

Please attend if you can if you are in Stony Brook University during the conference.

Vivek

 The proposed method of defining the OpenMP implementation of loop scheduling strategies is intended to be associated with in implementation of loop scheduling strategies in library’s such as that of LLVM’s OpenMP library and GCC’s library called libgomp.

The schedule for the technique in OpenMP's standard that is based on
the user specification rather predefined schedules ‘static’,
‘dynamic’, ‘guided’ .

      \item \tiny Work:rel: send message to John Damoulakis.  \te{1
        hour}. 

      \item \tiny Work:rel: talk to Andy Schmidt. \te{10 minutes} 

      \item \tiny Work:admin: give information for OpenMPCon to Claire. \te{1 hour} 

      \item \tiny Mgmt:Spaces: check that medical records were
        transferred. 

      \item \tiny Mgmt:wp: accept Jayanta Mukherjee. \te{20 minutes} 
