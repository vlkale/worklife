...
\item spaces: order additional iPhone chargers \timeEst{10 mins} \priority{U}
...
\item get a working technique that gives positive results (for load balanced codes). OR get a working technique that gives positve results for load imbalanced codes. \priority{UI}   (get Barnes-Hut working and try technique on that).

\item write up to Vk \timeEst{2 hours}.
\item add in results for transpose , send to everyone in slides. Add data layout and send to everyone  \priority{UI}

\item send Amanda the revised pthreads code, software description \priority{I}
\item To help define our idea and problem, add in related work. \timeEst{20 mins} \priority{U}
