...
\url{https://www.bnl.gov/newsroom/news.php?a=11892} 
  ...
  \begin{block}{Running ToDo} % find a better name 
    ...
    \item \tiny Work: do Cworks items = 
      Get application with ckloop that’s load imbalanced Modify ckloop to
      ...
      locality-optimized. Bind cores, i.e., drone mode.  Integrate with
      git. 
    ...
    \item \tiny Work: send message to Harshita
      ...
      I get the same error that you got when compiling charm on your branch
      using clang and specifying the compiler option -DCMK\_TASKQUEUE. 
      
      When I remove the argument memory\_order\_relaxed, lines 1757 and 1782
      of the file src/conv-core/convcore.c cause error messages still and
      charm doesn't compile successfully. Here is the exact error that I
      get:

      ../bin/charmc  -DCMK_TASKQUEUE  -I.   -c -o queueing.o queueing.C
      convcore.c:1757:53: error: too few arguments to function call,
      expected 3, have 2
    __atomic_add_fetch(&CsvAccess(idleThreadsCnt), 1);
      ~~~~~~~~~~~~~~~~~~                              ^
      convcore.c:1782:53: error: too few arguments to function call,
      expected 3, have 2
      __atomic_sub_fetch(&CsvAccess(idleThreadsCnt), 1);
      ~~~~~~~~~~~~~~~~~~                              ^
      1 warning and 2 errors generated.
      Fatal Error by charmc in directory
      /Users/vivekkale/Desktop/charm/multicore-darwin-x86_64/tmp
      Command clang -arch x86_64 -m64 -dynamic -fno-common
      -mmacosx-version-min=10.7 -D_REENTRANT -I../bin/../include
      -D__CHARMC__=1 -DCMK_TASKQUEUE -I. -fno-stack-protector -c
      ...
      -------------------------------------------------
      Charm++ NOT BUILT. Either cd into multicore-darwin-x86_64-gcc/tmp and
      try to resolve the problems yourself, visit
      http://charm.cs.illinois.edu/ for more information. Otherwise, email
      the developers at charm@cs.illinois.edu 
